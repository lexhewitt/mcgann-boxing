          ...
          <div className="grid grid-cols-7 gap-1">
            {buildMonthMatrix(new Date(monthAnchor.getFullYear(), monthAnchor.getMonth(), 1)).map((week, weekIdx) =>
              week.map(day => {
                ...
                return (
                  <div key={`${weekIdx}-${dayKey}`} className={`min-h-[110px] rounded-xl border px-2 py-1 flex flex-col gap-1 ${day.getMonth() === monthAnchor.getMonth() ? 'bg-white/5 border-gray-700' : 'bg-black/20 border-gray-800 opacity-50'}`}>
                    <div className="text-xs text-white font-semibold">{day.getDate()}</div>
                    <div className="flex flex-col gap-1 overflow-y-auto text-[11px] text-gray-200">
                      {entries.length === 0 && (
                      ...
                      {entries.map(slot => (
                        <button
                          ...
                          className="bg-brand-red/20 hover:bg-brand-red/40 rounded-md px-1 py-0.5 text-left text-white"
                        >
                          {formatTime(slot.start)} · £{slot.price.toFixed(0)}
                        </button>
                      ))}
                    </div>
                  </div>
